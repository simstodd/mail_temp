// V73_R_datagram_M0_TFT.ino sample code with my changes rf95_reliable_datagram_server.pde
...
//          To Do:
//          Time of Day/Date time not correct but seconds offset is correct
//          Mail seconds late resets with last packet...
//          At boot mailLastTime calculates way off
//          Add graphics for weather
//          Clean up code add packet counter
//
...
#define SERVER_ADDRESS 2 //the M0 is the server, like the gateway with TFT
...
int lastMail = 0;
int mailLastTime = 0;
int PreviousUnixTime = 946871427;
int now_alertTime = 0;
bool mail_yes = false;
...
delay(500);
Serial.println("V73_R_datagram_M0_TFT");
...
//  Serial.println(testFastLines(ILI9341_CYAN, ILI9341_PINK));
 ...
 tft.setRotation(1);
 tft.setCursor(0,0); //y,x
 tft.setTextColor(ILI9341_LIGHTGREY);  
 tft.setTextSize(1);
 tft.fillScreen(ILI9341_BLACK);
 delay(2000);
 tft.print("M0/TFT/LoRA receiver test# v73 ");
 tft.setTextSize(2);
tft.setTextColor(ILI9341_MAGENTA);
tft.setCursor(0,30);
tft.print("Testing radio");
tft.setTextColor(ILI9341_BLUE);
tft.setCursor(75,30);
tft.print(driver.printRegisters());
delay(2000);
tft.setCursor(0,60);
tft.setTextColor(ILI9341_GREEN);
tft.print("Radio test done..");
delay(1000);
tft.setCursor(0,90);
tft.setTextColor(ILI9341_CYAN);
tft.print("Waiting for first message..");
delay(3000);
...
uint8_t data[] = "Hello litle 32u4 from v73 M0 server (0x2) with TFT";
 ...
 if (manager.available())
  {
    ...
    if (manager.recvfromAck(buf, &len, &from))
    {
      ...
      displayTextW(); //write to TFT
    DateTime now = rtc.now();
    Serial.print("RSSI: ");
    Serial.println(driver.lastRssi()); //get RSSI
    ...
    }//end if manager rec from ack
  }// end if manager
 }//end main loop

void displayTextW()
{
    //received message so displaying message 
    DateTime now_Message = rtc.now(); //get current time
    int ElapsedUnixTime = now_Message.unixtime() - PreviousUnixTime; //get current seconds offset
    PreviousUnixTime = now_Message.unixtime(); //reset PreviousTime
    String myString = (char*)buf;
    tft.setRotation(1);
   // tft.fillScreen(ILI9341_BLACK);
  ...
  tft.setCursor(0,180); //near bottom
  tft.setTextColor(ILI9341_CYAN);
  tft.setTextSize(2);
  tft.print("Last Tx:  ");
  tft.print(ElapsedUnixTime);
  tft.print(" secs ago");
  //last Tx received
    ...
    tft.setCursor(0, 120);
    tft.println("Humid:");
    tft.setTextColor(ILI9341_PINK);//pink
