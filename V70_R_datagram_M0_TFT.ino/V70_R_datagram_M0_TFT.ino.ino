// V70_R_datagram_M0_TFT.ino sample code with my changes rf95_reliable_datagram_server.pde
...
delay(500);
Serial.println("V70_R_datagram_M0_TFT");
...
//  Serial.println(testFastLines(ILI9341_CYAN, ILI9341_PINK));
 ...
 tft.fillScreen(ILI9341_BLACK);
   delay(4000);
  tft.print("M0/TFT/LoRA receiver test# v70 ");
  delay(4000);
  tft.fillScreen(ILI9341_BLACK);
  tft.setCursor(50,50);
  tft.print("M0/TFT/LoRA receiver test# v70 ");
...
uint8_t data[] = "Hello litle 32u4 from v70 M0 server (0x2) with TFT";
     ...
     //done setAlert State 
      ...
      //examine
          if (myString.substring(10, 11) > "0") {
                      alertState = HIGH;
                      Serial.print("Mailbox motion at: ");
                      DateTime mail_last = rtc.now();
...
//last message       
  ...
  //  tft.println(myString.substring(22, 27));
   ...
   //tft.setFont(&FreeSerif12pt7b);
    //examine
          if (alertState = HIGH){ //global variable alertState
            //if (myString.substring(10, 11) > "0") {
            //tft.println(myString.substring(10, 11));
            lastMail = now.unixtime(); //last time mail action occured
                 
            tft.print("Mail happened: ");
            tft.setCursor(190, 160);
            //tft.setTextColor(ILI9341_WHITE);  tft.setTextSize(2);
            //tft.print(mail_last.hour(), DEC);
           // tft.print(':');
           // tft.print(mail_last.minute(), DEC);
           // tft.print(':');
           // tft.println(mail_last.second(), DEC);
        }//end examine alert string 

//display mail time elapsed in seconds
              //do i need rtc call here?
              int mailcalc = now.unixtime();
              int maillastTime = mailcalc - lastMail;
              tft.setCursor(190, 160);
              tft.setTextColor(ILI9341_WHITE);  tft.setTextSize(2);
              tft.print(maillastTime);
              tft.print(" seconds ago");
//mail happened ago
